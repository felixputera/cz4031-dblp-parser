...
class PhdThesisHandler(xml.sax.handler.ContentHandler):
    is_phd_thesis = False
    new_row = {}
    new_row_parent = {}
    csv_writer = CsvWriter(fieldnames=('id', 'title', 'school', 'year', 'type', 'address', 'month', 'note', 'author'), fieldnames_parent=('id', 'key', 'mdate', 'publtype'), fileout='phdthesis.txt', fileout_parent='publication.txt')
    is_title = False
    is_school = False
    is_year = False
    is_type = False
    ...
    def startElement(self, name, attrs):
        if not self.is_phd_thesis:
            if name == 'phdthesis':
                self.is_phd_thesis = True
